...
def draw_text_cn(
    img_bgr, text, xy, font_size=20, color=(255, 255, 255), stroke=None, ui_hint=True
):
    """
    统一的文本绘制：
...
def draw_progress_bars(vis, align_score, range_score):
    ...
    cv2.rectangle(
        vis,
        (x0, y0),
        (x0 + int(bar_w * clamp01(align_score)), y0 + bar_h),
        (0, 220, 0),
        -1,
    )
    cv2.rectangle(
...
def draw_measure_arrow(img, p1, p2, txt=None):
    ...
    cv2.putText(
        img, txt, (x0 + pad, y1 - 6), font, fs, (255, 255, 255), th, cv2.LINE_AA
    )
...
def main(headless: bool = False, prompt_name: str = None, stop_event=None):
    ...
    print(
        f"[CLASS] target id={target_cls_id}, name={id_to_name.get(target_cls_id, 'N/A')}"
    )
    ...
    try:
        while True:
            ...
            if MODE == "SEGMENT":
                ...
                if candidate_masks:
                    ...
                    if len(candidate_masks) > 1:
                        draw_text_cn(
                            vis,
                            f"检测到{len(candidate_masks)}个物体，选择最大的（面积: {largest_mask_info['area']}）",
                            (10, H - 30),
                            font_size=16,
                            color=(255, 255, 0),
                        )
                ...
                if detected_object and last_detected_mask is not None:
                    if auto_lock_start_time is None:
                        ...
                        print(
                            f"[AUTO] 检测到物体，选择最大的（面积: {np.sum(last_detected_mask)}），开始倒计时..."
                        )
                ...
                if flash_start_time is not None and flash_mask is not None:
                    ...
                    if elapsed < flash_duration:
                        ...
                        if contours:
                            # 轮廓颜色也跟随alpha变化
                            contour_color = tuple(
                                int(c * (0.5 + alpha * 0.5)) for c in flash_color
                            )
                            cv2.drawContours(
                                vis, contours, -1, contour_color, STROKE_WIDTH + 1
                            )
                                ...
                                if (
                                    ...
                                    if det["masks"]:
                                        ...
                                        inter = np.logical_and(
                                            new_det_mask, poly_mask
                                        ).sum()
                                        union = (
                                            np.logical_or(new_det_mask, poly_mask).sum()
                                            + 1e-6
                                        )
                                        iou = inter / union
                                        ...
                                        if (
                                            ...
                                            if pts is not None and len(pts) >= 8:
                                                ...
                                                flow_grace = 0
                                                print(
                                                    "[RESEED] YOLOE 低 IoU 触发重播种（已更新光流特征点）"
                                                )
                            ...
                            if len(poly) >= 3:
                                ...
                                if use_yoloe and yoloe_backend is not None:
                                    ...
                                    if track_frame_count % 30 == 0:  # 每30帧打印一次
                                        print(
                                            f"[YOLOE] TRACK模式实时检测第 {track_frame_count} 帧"
                                        )
                                    ...
                                    if det["masks"]:
                                        ...
                                        poly_mask = np.zeros((H, W), dtype=np.uint8)
                                        cv2.fillPoly(
                                            poly_mask, [poly.astype(np.int32)], 1
                                        )
                                        inter = np.logical_and(
                                            latest_det_mask, poly_mask
                                        ).sum()
                                        union = (
                                            np.logical_or(
                                                latest_det_mask, poly_mask
                                        ...
                                        # 降低IoU阈值，更积极地校正
                                        if (
                                            iou > YOLO_CORRECTION_IOU_THRESHOLD
                                        ):  # 使用可配置阈值
                                            # 用 YOLOE 结果更新多边形
                                            contours, _ = cv2.findContours(
                                                latest_det_mask,
                                ...
                                overlap_ratio = 0.0
                                if hand_box is not None and poly is not None:
                                    is_touching, overlap_ratio = (
                                        check_hand_object_contact(
                                            hand_box, poly, overlap_threshold=0.1
                                        )
                                    )
                                ...
                                if is_touching:
                                    ...
                                    overlay = vis.copy()
                                    cv2.fillPoly(
                                        overlay, [poly.astype(np.int32)], (0, 255, 0)
                                    )
            ...
            fps = (
                0.0
                if len(fps_hist) < 2
                else (len(fps_hist) - 1) / (fps_hist[-1] - fps_hist[0])
            )
            draw_text_cn(
                vis,
                f"FPS: {fps:.1f}",

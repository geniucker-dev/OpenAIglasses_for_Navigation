...
from crosswalk_awareness import (
    CrosswalkAwarenessMonitor,
    split_combined_voice,
)  # 斑马线感知
...
class BlindPathNavigator:
    ...
    def __init__(self, yolo_model=None, obstacle_detector=None):
        ...
        self.guide_interval = float(
            os.getenv("AIGLASS_STRAIGHT_INTERVAL", "4.0")
        )  # 播报间隔（秒）
        self.last_guide_time = 0.0
        self.straight_continuous_mode = (
            os.getenv("AIGLASS_STRAIGHT_CONTINUOUS", "1") == "1"
        )  # 持续播报模式
        self.straight_repeat_limit = int(
            os.getenv("AIGLASS_STRAIGHT_LIMIT", "2")
        )  # 限制模式下的最大次数
        self.straight_repeat_count = 0

        # 【新增】方向指令持续播报配置
        self.direction_interval = float(
            os.getenv("AIGLASS_DIRECTION_INTERVAL", "3.0")
        )  # 方向指令间隔（秒）
        ...
        # 打印配置信息
        logger.info(
            f"[BlindPath] 直行播报配置: 间隔={self.guide_interval}秒, "
        ...
        self.OBSTACLE_DETECTION_INTERVAL = int(
            os.getenv("AIGLASS_OBS_INTERVAL", "15")
        )  # 默认每5帧检测一次
        self.OBSTACLE_CACHE_DURATION_FRAMES = int(
            os.getenv("AIGLASS_OBS_CACHE_FRAMES", "10")
        )  # 缓存10帧
        ...
        # 【新增】盲道YOLO检测间隔
        self.BLINDPATH_DETECTION_INTERVAL = int(
            os.getenv("AIGLASS_BLINDPATH_INTERVAL", "8")
        )  # 每2帧检测一次
        ...
        logger.info(
            f"[BlindPath] 盲道检测间隔: 每{self.BLINDPATH_DETECTION_INTERVAL}帧"
        )
    ...
    @staticmethod
    def _demo_fallback_mask(image: np.ndarray) -> np.ndarray:
        h, w = image.shape[:2]
    ...
    def _detect_traffic_light_by_color(self, image: np.ndarray) -> str:
        ...
        if hasattr(self, "frame_counter") and self.frame_counter % 30 == 0:
            ...
            if os.getenv("AIGLASS_DEBUG_TRAFFIC_LIGHT", "0") == "1":
                ...
                cv2.imwrite(
                    f"{debug_dir}/frame_{self.frame_counter}_green.jpg", mask_green
                )
                cv2.imwrite(
                    f"{debug_dir}/frame_{self.frame_counter}_yellow.jpg", mask_yellow
                )
    ...
    def _get_voice_priority(self, guidance_text):
        ...
        # 保持直行 - 最低优先级
        if (
            "保持直行" in guidance_text
            or "继续前进" in guidance_text
            or "方向正确" in guidance_text
        ):
            return 10
    ...
    def process_frame(self, image: np.ndarray) -> ProcessingResult:
        ...
        if self.frame_counter % 30 == 0:  # 每30帧打印一次
            ...
            logger.info(
                f"[YOLO检测] Frame={self.frame_counter}, 盲道={'有' if has_blind else '无'}, "
        ...
        if self.frame_counter % 30 == 0 and crosswalk_mask_before_stabilize is not None:
            ...
            logger.info(
                f"[掩码稳定化] 斑马线稳定化后: {'有' if after_stab else '无（被过滤）'}"
            )
        ...
        if self.frame_counter % self.OBSTACLE_DETECTION_INTERVAL == 0:
            ...
            logger.info(
                f"[Frame {self.frame_counter}] 执行了新的障碍物检测，检测到 {len(detected_obstacles)} 个障碍物"
            )
        else:
            if (
                ...
                logger.info(
                    f"[Frame {self.frame_counter}] 使用缓存的障碍物数据，共 {len(detected_obstacles)} 个障碍物"
                )
            else:
                detected_obstacles = []
        ...
        for i, obs in enumerate(detected_obstacles):
            logger.info(
                f"  障碍物 {i + 1}: {obs.get('name', 'unknown')}, "
        ...
        if crosswalk_mask is not None:
            ...
            if cross_pixels > 0:
                logger.info(
                    f"[斑马线] 传入monitor: pixels={cross_pixels}, area={cross_pixels / crosswalk_mask.size * 100:.2f}%"
                )
            else:
        ...
        crosswalk_guidance = self.crosswalk_monitor.process_frame(
            crosswalk_mask, blind_path_mask
        )
        if crosswalk_guidance:
            logger.info(
                f"[斑马线感知] 检测结果: area={crosswalk_guidance.get('area', 0):.3f}, "
        ...
        if crosswalk_guidance and crosswalk_guidance["should_broadcast"]:
            ...
            logger.info(
                f"[斑马线语音] 已设置待播报语音: {crosswalk_guidance['voice_text']}, 优先级{crosswalk_guidance['priority']}"
            )
        ...
        if False:  # current_stage == 'ready':
            ...
            if self.frame_counter % 30 == 0:  # 每30帧打印一次
                logger.info(
                    f"[红绿灯检测] 当前颜色: {traffic_light_color}, 历史: {list(self.traffic_light_history)}"
                )
        ...
        if voice_candidates:
            ...
            if final_guidance_text == "保持直行":
                if self.straight_continuous_mode:
                    ...
                    # 原有的限制模式
                    if (
                        current_time - self.last_guide_time >= self.guide_interval
                    ) and (self.straight_repeat_count < self.straight_repeat_limit):
                        self.last_guide_time = current_time
                        self.straight_repeat_count += 1
                        self.last_any_speech_time = current_time
                    else:
                ...
                is_direction = any(
                    keyword in final_guidance_text for keyword in direction_keywords
                )
                ...
                if is_direction:
                    ...
                    if final_guidance_text == self.last_direction_message:
                        # 同一个方向指令，检查时间间隔
                        if (
                            current_time - self.last_direction_time
            ...
            if final_guidance_text:
                try:
                    ...
                    if (
                        ...
                        and "," in final_guidance_text
                    ):
                        voice_parts = split_combined_voice(final_guidance_text)
                        logger.info(
                            f"[斑马线语音] 组合播报检测到{len(voice_parts)}部分，只播第一部分保持实时"
                        )
                        # 只播放第一部分，后续部分丢弃以保持实时性
                        if voice_parts:
                            play_voice_text(voice_parts[0])
                            logger.info(
                                f"[语音播报] 优先级{selected_voice['priority']}: {voice_parts[0]}"
                            )
                    else:
                        play_voice_text(final_guidance_text)
                        logger.info(
                            f"[语音播报] 优先级{selected_voice['priority']}: {final_guidance_text}"
                        )
                except Exception as e:
        ...
        if frame_visualizations:
            annotated_image = self._draw_visualizations(
                image.copy(), frame_visualizations
            )
        else:
        ...
        current_instruction = (
            final_guidance_text if final_guidance_text else "等待中..."
        )
        annotated_image = self._draw_command_button(
            annotated_image, current_instruction
        )
    ...
    def _detect_path_and_crosswalk(
        self, image: np.ndarray
    ) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
        """检测盲道和斑马线"""
        if self.yolo_model is None:
            logger.warning("YOLO模型未加载")
            if os.getenv("AIGLASS_BLINDPATH_DEMO_MASK", "0") == "1":
                logger.warning("[BlindPath] 使用演示掩码回退")
                return self._demo_fallback_mask(image), None
            return None, None
        ...
        try:
            ...
            if (
                ...
                for mask_tensor, conf_tensor, cls_tensor in zip(
                    ...
                    class_id = int(self._scalar(cls_tensor, default=-1))
                    confidence = float(self._scalar(conf_tensor, default=0.0))
                    threshold = self.CLASS_CONF_THRESHOLDS.get(class_id, 1.0)

                    if confidence >= threshold:
                        current_mask = self._tensor_to_mask(
                            mask_tensor, image.shape[1], image.shape[0]
                        )
                        ...
                        if class_id == 1:  # 盲道
                            if blind_path_mask is None:
                                ...
                                blind_path_mask = cv2.bitwise_or(
                                    blind_path_mask, current_mask
                                )
                        elif class_id == 0:  # 斑马线
                            if crosswalk_mask is None:
                                ...
                                crosswalk_mask = cv2.bitwise_or(
                                    crosswalk_mask, current_mask
                                )
        except Exception as e:
            logger.error(f"YOLO检测失败: {e}")
            if os.getenv("AIGLASS_BLINDPATH_DEMO_MASK", "0") == "1":
                logger.warning("[BlindPath] 检测失败后使用演示掩码回退")
                blind_path_mask = self._demo_fallback_mask(image)
        ...
        try:
            ...
            if not isinstance(mask_tensor, torch.Tensor):
                ...
                if arr.dtype != np.uint8:
                    arr = (
                        (arr > 0.5).astype(np.uint8) * 255
                        if binarize
                        else (arr * 255.0).astype(np.uint8)
                    )
                mask_u8 = arr
            else:
                ...
                if binarize:
                    ...
                    mask_u8 = (
                        (mask_tensor.mul(255).clamp_(0, 255).to(torch.uint8))
                        .cpu()
                        .numpy()
                    )
    ...
    def _stabilize_mask(
        self, prev_gray, curr_gray, raw_mask, prev_stable_mask, mask_type
    ):
    ...
    def _stabilize_seg_mask(
        ...
        if prev_mask_b is None or prev_gray is None or curr_gray is None:
            return (
                _morph_smooth(curr_mask_b, morph_kernel)
                if curr_mask_b is not None
                else None
            )
        ...
        if curr_mask_b is not None and np.sum(curr_mask_b > 0) >= min_area_px:
            ...
            if prev_mask_b is not None:
                ...
                # IoU较低但仍有重叠，进行加权融合
                    ...
                    # 使用光流预测的掩码
                    flow_mask = self._predict_mask_with_flow(
                        prev_mask_b, prev_gray, curr_gray
                    )
                    if flow_mask is not None:
                        ...
                        w_curr = min(
                            0.9, 0.4 + iou
                        )  # IoU=0.1时w_curr=0.5, IoU=0.5时w_curr=0.9
                        w_flow = 1.0 - w_curr

                        fused = w_curr * curr_mask_b.astype(
                            np.float32
        ...
        # 当前帧没有检测结果，尝试使用光流外推
            ...
            if (
                prev_mask_b is not None
                and np.sum(prev_mask_b > 0) >= min_area_px
                and ttl > 0
            ):
                # 使用光流预测
                flow_mask = self._predict_mask_with_flow(
    ...
    def _predict_mask_with_flow(self, prev_mask, prev_gray, curr_gray):
        ...
        try:
            ...
            p0 = cv2.goodFeaturesToTrack(
                prev_gray, mask=edge_mask, **self.feature_params
            )
            ...
            for hist_idx, hist_data in enumerate(
                ...
                if len(y_diffs) > 0:
                    ...
                    if y_diffs[closest_idx] < 10:  # y坐标差异小于10像素
                        ...
                        # 历史帧权重递减
                        weights.append(
                            0.5 ** (len(self.centerline_history) - hist_idx - 1)
                        )
    ...
    def _add_mask_visualization(
        self, mask, visualizations, viz_type, color, add_outline=True
    ):
        """添加掩码可视化（增加描边）"""
        if mask is None:
            return
        ...
        try:
            ...
            if contours:
                ...
                visualizations.append(
                    {"type": viz_type, "points": points, "color": color}
                )
    ...
    def _update_crosswalk_tracker(self, crosswalk_mask, image_height, image_width):
        ...
        if crosswalk_mask is not None:
            ...
            if len(y_coords) > 0:
                ...
                if (
                    is_ready_to_switch
                    and self.crosswalk_tracker["alignment_status"] == "aligned"
                ):
    ...
    def _handle_crosswalk_approaching(
        self, frame_visualizations, image_height, image_width, image
    ):
        """处理接近斑马线的情况"""
        # 障碍物检测
        if (
            self.obstacle_detector
            and self.frame_counter % self.OBSTACLE_DETECTION_INTERVAL == 0
        ):
        ...
        if near_obstacles:
            ...
            if obstacle_name != self.last_obstacle_speech:
                ...
                current_time - self.last_obstacle_speech_time
                > self.obstacle_speech_cooldown
            ):
                should_announce = True
                self.last_obstacle_speech_time = current_time
        ...
        if vp_features and vp_features["fit_error"] < self.VP_FIT_ERROR_THRESHOLD:
            ...
            if self.onboarding_step == ONBOARDING_STEP_ROTATION:
                if abs(VP[0] - image_center_x) < (
                    ...
                    guidance_text = (
                        "请向左转动。" if VP[0] < image_center_x else "请向右转动。"
                    )
                ...
                L_center_bottom_x = self._calculate_line_x_at_y(
                    L_center, image_height - 1
                )
                ...
                if L_center_bottom_x:
                    ...
                    if (
                        center_offset_ratio
                        < self.ONBOARDING_CENTER_OFFSET_THRESHOLD_RATIO
                    ):
    ...
    def _handle_maneuvering_turn(
        ...
        if self.maneuver_step == MANEUVER_STEP_1_ISSUE_COMMAND:
            direction_text = (
                "右" if self.maneuver_target_info["direction"] == "right" else "左"
            )
    ...
    def _handle_locking_on(self, frame_visualizations):
        ...
        # 添加脉冲特效
        self._add_obstacle_visualization(
            main_obstacle, frame_visualizations, pulse_effect=True
        )
    ...
    def _handle_avoiding_obstacle(
        ...
        """处理避障状态"""
        if not self.avoidance_plan or self.avoidance_step_index >= len(
            self.avoidance_plan
        ):
            self.current_state = STATE_NAVIGATING
            self.avoidance_plan = None
        ...
        if step["type"] == "sidestep_clear":
            ...
            if final_obstacles:
                guidance_text = (
                    f"路径被挡住，请向{'右' if direction == 'right' else '左'}侧平移。"
                )
            else:
    ...
    def _get_vanishing_point_features(self, mask):
        ...
        try:
            ...
            [vx_l, vy_l, x_l, y_l] = cv2.fitLine(
                left_points, cv2.DIST_L2, 0, 0.01, 0.01
            )
            [vx_r, vy_r, x_r, y_r] = cv2.fitLine(
    ...
    def _detect_sharp_corner(self, centerline_data, angle_threshold_deg=45):
        ...
        try:
            ...
            for i in range(0, num_points - 2 * window_size, 2):
                ...
                if (
                    angle_diff_deg > max_angle_diff
                    and angle_diff_deg > angle_threshold_deg
                ):
                    ...
                    post_turn_segment = points_in_range[
                        corner_point_idx : min(
                            corner_point_idx + window_size * 2, num_points
                        )
                    ]
    ...
    def _detect_obstacles(self, image, path_mask=None):
        """检测障碍物"""
        logger.info(
            f"[_detect_obstacles] 开始执行，Frame={self.frame_counter}, obstacle_detector={'已加载' if self.obstacle_detector else '未加载'}"
        )
        ...
        if not hasattr(self, "_classes_printed"):
            ...
            if hasattr(self.obstacle_detector, "WHITELIST_CLASSES"):
                ...
                logger.info(
                    f"[_detect_obstacles] 总共 {len(self.obstacle_detector.WHITELIST_CLASSES)} 个类别"
                )
        ...
        try:
            ...
            logger.info(
                f"[_detect_obstacles] 调用ObstacleDetectorClient.detect()... image.shape={image.shape}"
            )
            detected_obstacles = self.obstacle_detector.detect(
                image, path_mask=path_mask
            )

            logger.info(
                f"[_detect_obstacles] ObstacleDetectorClient 返回 {len(detected_obstacles)} 个物体"
            )
                ...
                if "mask" in obj and obj["mask"] is not None:
                    ...
                    if len(y_coords) > 0 and len(x_coords) > 0:
                        ...
                        if "confidence" not in obj:
                            obj["confidence"] = (
                                0.5  # ObstacleDetectorClient 已经过滤了低置信度的
                            )

                # 详细的日志输出
                logger.info(
                    f"[_detect_obstacles] 物体 {i + 1}/{len(detected_obstacles)}: "
                )
                logger.info(f"  - 类别: {obj.get('name', 'unknown')}")
                logger.info(
                    f"  - 面积: {obj.get('area', 0)} pixels ({obj.get('area_ratio', 0):.3f} of image)"
                )
                logger.info(
                    f"  - 中心点: ({obj.get('center_x', 0):.1f}, {obj.get('center_y', 0):.1f})"
                )
                logger.info(
                    f"  - bottom_y_ratio: {obj.get('bottom_y_ratio', 0):.3f} (是否近距离: {'是' if obj.get('bottom_y_ratio', 0) > 0.7 else '否'})"
                )
                logger.info(
                    f"  - area_ratio: {obj.get('area_ratio', 0):.3f} (是否过大: {'是' if obj.get('area_ratio', 0) > 0.1 else '否'})"
                )
            ...
            logger.info(
                f"[_detect_obstacles] ===== 最终结果: 返回 {len(detected_obstacles)} 个障碍物 ====="
            )
            for idx, obj in enumerate(detected_obstacles):
                logger.info(
                    f"  {idx + 1}. {obj.get('name', 'unknown')} - 位置:({obj.get('center_x', 0):.0f},{obj.get('center_y', 0):.0f}) "
    ...
    def _handle_pixel_domain_onboarding(
        ...
        if self.onboarding_step == ONBOARDING_STEP_ROTATION:
            if abs(orientation_error_rad) < self.ONBOARDING_ORIENTATION_THRESHOLD_RAD:
                ...
                guidance_text = (
                    "请向左转动。" if orientation_error_rad > 0.1 else "请向右转动。"
                )
            ...
            if center_offset_ratio < self.ONBOARDING_CENTER_OFFSET_THRESHOLD_RATIO:
                ...
                guidance_text = (
                    "请向右平移。" if center_offset_pixels > 0 else "请向左平移。"
                )
    ...
    def _add_data_panel(self, visualizations, data, position):
        """添加数据面板"""
        visualizations.append(
            {"type": "data_panel", "data": data, "position": position}
        )
    ...
    def _add_traffic_light_visualization(
        self, color, visualizations, image_height, image_width
    ):
        """添加红绿灯状态可视化"""
        # 在右上角绘制红绿灯指示器
        x = image_width - 100
    ...
    def _draw_command_button(self, image, text):
        ...
        try:
                ...
                try:
                    ...
                    if font:
                        ...
                        (tw, th), _ = cv2.getTextSize(
                            full_text, cv2.FONT_HERSHEY_SIMPLEX, scale, 1
                        )
                except:
                    scale = font_px / 24.0
                    (tw, th), _ = cv2.getTextSize(
            ...
            # 圆角矩形（中间+两个圆）
            cv2.rectangle(
                overlay, (left + radius, top), (right - radius, bottom), bg_color, -1
            )
            cv2.circle(
                overlay, (left + radius, (top + bottom) // 2), radius, bg_color, -1
            )
            cv2.circle(
                overlay, (right - radius, (top + bottom) // 2), radius, bg_color, -1
            )
            ...
            # 绘制边框
            cv2.rectangle(
                image, (left + radius, top), (right - radius, bottom), border_color, 1
            )
            cv2.circle(
                image, (left + radius, (top + bottom) // 2), radius, border_color, 1
            )
            cv2.circle(
                image, (right - radius, (top + bottom) // 2), radius, border_color, 1
            )
    ...
    def _draw_data_panel_no_bg(self, image, data, position=(15, 15)):
        ...
        try:
            ...
            for key, value in data.items():
                ...
                # 绘制白色文字
                draw.text(
                    (position[0], y_offset), text, font=font, fill=(255, 255, 255, 255)
                )
    ...
    def _draw_visualizations(self, image, viz_elements):
            ...
            if elem_type in ["blind_path_mask", "obstacle_mask", "crosswalk_mask"]:
                ...
                if points.size > 0:
                    color = self._parse_color(
                        element.get("color", "rgba(255, 255, 255, 0.5)")
                    )

                    # 处理脉冲效果
                    if element.get("effect") == "pulse":
                        pulse_speed = element.get("pulse_speed", 1.0)
                        alpha = 0.3 + 0.3 * np.sin(
                            current_time * pulse_speed * 2 * np.pi
                        )
                    else:
                        alpha = 0.4
        ...
        for element in standard_elements:
            ...
            # 绘制折线
                ...
                if points.size > 0:
                    color = self._parse_color(
                        element.get("color", "rgba(255, 255, 0, 1.0)")
                    )
            ...
            # 绘制箭头
                ...
                color = self._parse_color(
                    element.get("color", "rgba(0, 255, 255, 1.0)")
                )
                thickness = element.get("thickness", 2)
                tip_length = element.get("tip_length", 0.3)
                cv2.arrowedLine(
                    image, start, end, color, thickness, tipLength=tip_length
                )
            ...
            # 【新增】绘制虚线
                ...
                color = self._parse_color(
                    element.get("color", "rgba(255, 255, 255, 0.6)")
                )
                thickness = element.get("thickness", 2)
                dash_length = 10
                gap_length = 5
                ...
                if total_len > 0:
                    ...
                    while current_len < total_len:
                        ...
                        seg_end = start + unit_vec * min(
                            current_len + dash_length, total_len
                        )
                        cv2.line(
                            image,
            ...
            # 【新增】绘制角度弧线
                ...
                color = self._parse_color(
                    element.get("color", "rgba(255, 200, 0, 0.8)")
                )
                thickness = element.get("thickness", 2)
        ...
        if PIL_AVAILABLE:
            for panel in panel_elements:
                image = self._draw_data_panel_no_bg(
                    image, panel["data"], panel["position"]
                )
        else:
            # 如果没有PIL，也使用描边效果
            for panel in panel_elements:
    ...
    def _draw_chinese_text(
        ...
        if not PIL_AVAILABLE:
            ...
            cv2.putText(
                image, text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, 2
            )
            return image
        ...
        for curr_obs in obstacles:
            ...
            if prev_obstacles:
                for idx, prev_obs in enumerate(prev_obstacles):
                    ...
                    # 使用光流预测历史障碍物的新位置
                    flow_mask = self._predict_mask_with_flow(
                        prev_obs["mask"], prev_gray, curr_gray
                    )
            ...
            if best_match is not None and best_idx >= 0:
                ...
                # 融合当前检测和光流预测，提高稳定性
                fused_mask = ((0.8 * curr_mask + 0.2 * best_match) > 128).astype(
                    np.uint8
                ) * 255

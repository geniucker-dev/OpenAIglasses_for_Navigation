...
# 音频文件映射（由 voice 目录的 map.zh-CN.json 动态加载）
AUDIO_MAP = {}


def _build_voice_map_from_files():
    added = 0
    try:
        for fname in os.listdir(VOICE_DIR):
            ...
            if ext.lower() != ".wav":
                continue
            if stem.endswith("_原始"):
                continue
            if stem not in AUDIO_MAP:
                AUDIO_MAP[stem] = fpath
                added += 1
    except Exception as e:
        print(f"[AUDIO] 通过文件名构建基础映射失败: {e}")
        return 0
    print(f"[AUDIO] 已按文件名回退构建基础映射 {added} 条")
    return added
...
def _merge_voice_map():
    ...
    try:
        if not os.path.exists(VOICE_MAP_FILE):
            ...
            return _build_voice_map_from_files()

        ...
        while True:
            ...
            if frame is None:
                ...
                if current_time - last_frame_log_time > 3.0:
                    print(
                        f"[TRAFFIC] 帧统计: 总={frame_count}, 收到={frame_received_count}, "
            ...
            if results and len(results) > 0:
                ...
                if r.boxes is not None and len(r.boxes) > 0:
                    ...
                    for box in r.boxes:
                        ...
                        # 确定颜色
                        color = LIGHT_COLORS.get(
                            class_name_lower, FRONTEND_COLORS["text"]
                        )
                        ...
                        if _PIL_OK and _FONT_PATH:
                            try:
                                ...
                                bg_y2 = label_y + 4
                                cv2.rectangle(
                                    vis, (bg_x1, bg_y1), (bg_x2, bg_y2), color, -1
                                )
            ...
            if len(detection_history) >= MAJORITY_THRESHOLD:
                # 统计最近N帧中每个状态出现的次数
                valid_detections = [
                    d for d in detection_history if d and d in LIGHT_NAMES
                ]
                if len(valid_detections) >= MAJORITY_THRESHOLD:
                    ...
                    if most_common and most_common[0][1] >= MAJORITY_THRESHOLD:
                        ...
                        if frame_received_count % 30 == 0:
                            print(
                                f"[TRAFFIC] 检测历史: {detection_history[-5:]}, 稳定状态: {stable_light}"
                            )
            ...
            if stable_light:
                ...
                if stable_light != last_detected_light:
                    ...
                    print(
                        f"[TRAFFIC] 检测到稳定状态改变: {LIGHT_NAMES[stable_light]}（不播报）"
                    )
                    last_tts_ts = t_now
                # 超过间隔时间，更新时间戳（但不播报）
                elif (t_now - last_tts_ts) > TTS_INTERVAL_SEC:
                    print(
                        f"[TRAFFIC] 稳定状态持续: {LIGHT_NAMES[stable_light]}（不播报）"
                    )
                    last_tts_ts = t_now

...
ANGLE_THRESH_DEG = float(
    os.getenv("CROSSWALK_ANGLE_THRESH_DEG", "5.0")
)  # 默认阈值略放宽
...
def _looks_like_blind_path(
    bp_mask: np.ndarray, cw_mask: np.ndarray, H: int, W: int
) -> bool:
...
class CrossStreetNavigator:
    ...
    def __init__(
        ...
        self.last_blindpath_announce_time = (
            0  # 上次播报盲道提示的时间（用于节流重复播报）
        )
        ...
        self.OBSTACLE_CACHE_DURATION_FRAMES = int(
            os.getenv("AIGLASS_OBS_CACHE_FRAMES", "0")
        )

        # 【新增】斑马线检测间隔配置
        self.CROSSWALK_DETECTION_INTERVAL = int(
            os.getenv("AIGLASS_CROSSWALK_INTERVAL", "4")
        )  # 每4帧检测一次
        ...
        if self.obstacle_detector is None and os.getenv("AIGLASS_OBS_AUTO", "1") != "0":
            try:
                if ObstacleDetectorClient is not None:
                    ...
                    logger.warning(
                        "[CROSS_STREET] 未找到 ObstacleDetectorClient，跳过自动加载"
                    )
    ...
    # —— 打点/追踪辅助 ——
    @staticmethod
    def _inner_offset_edge(
        mask_bin: np.ndarray, offset_px=5, edge_dilate_px=2
    ) -> np.ndarray:
        """对二值掩码做内收后提边缘，便于在目标内部打光流特征点"""
        if offset_px > 0:
            k = cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE, (2 * offset_px + 1, 2 * offset_px + 1)
            )
    ...
    @staticmethod
    def _hull_mask_from_points(
        points: np.ndarray, shape_hw: tuple
    ) -> Optional[np.ndarray]:
        """从一组点的凸包生成二值掩码"""
    ...
    def _estimate_angle_by_stripes(
        ...
        try:
            ...
            for x1, y1, x2, y2 in lines.reshape(-1, 4):
                ...
                weights.append(w)
                all_lines.append(
                    (int(x1), int(y1 + roi_top), int(x2), int(y2 + roi_top))
                )
    ...
    def _get_crosswalk_guidance_features(
        self, mask: np.ndarray, image_shape: tuple
    ) -> dict:
    ...
    def process_frame(self, bgr_image: np.ndarray) -> CrossStreetResult:
        ...
        try:
            ...
            if (
                self.seg_model
                and self.frame_counter % self.CROSSWALK_DETECTION_INTERVAL == 0
            ):
                # 执行新的检测
                # 使用较低的基础阈值获取所有候选
                base_thr = min(CROSSWALK_MIN_CONF, BLIND_MIN_CONF)
                detections = (
                    self.seg_model.detect(bgr_image, confidence_threshold=base_thr)
                    or []
                )
                ...
                if (
                    ...
                    try:
                        ...
                        if p1 is not None and st is not None:
                            good_new = p1[st == 1]
                            if (
                                good_new is not None
                                and len(good_new) >= TRACK_MIN_POINTS
                            ):
                                tracked_mask = self._hull_mask_from_points(
                                    good_new, (h, w)
                                )
                                if tracked_mask is not None and int(
                                    tracked_mask.sum()
                                ) >= (
                                    0.3
                                    * (
            ...
            if crosswalk_mask is not None:
                ...
                if (
                    self.prev_mask_float is not None
                    and self.prev_mask_float.shape == m.shape
                ):
                    self.prev_mask_float = (
                        MASK_EMA_ALPHA * m
                        + (1.0 - MASK_EMA_ALPHA) * self.prev_mask_float
                    )
                else:
            ...
            if self.obstacle_detector is not None:
                if self.frame_counter % self.OBSTACLE_DETECTION_INTERVAL == 0:
                    detected_obstacles = self._detect_obstacles(
                        bgr_image, path_mask=crosswalk_mask
                    )
                    # 稳定化
            ...
            if blindpath_mask is not None:
                ...
                for c in range(3):
                    annotated[:, :, c] = np.where(
                        mask_area,
                        (annotated[:, :, c] * 0.7 + green_color[c] * 0.3).astype(
                            np.uint8
                        ),
                        annotated[:, :, c],
                    )
            ...
            if crosswalk_mask is not None:
                ...
                for c in range(3):
                    annotated[:, :, c] = np.where(
                        mask_area,
                        (annotated[:, :, c] * 0.7 + orange_color[c] * 0.3).astype(
                            np.uint8
                        ),
                        annotated[:, :, c],
                    )
            ...
            if self.state == STATE_SEEKING:
                ...
                if crosswalk_mask is not None:
                    ...
                    if is_near:
                        ...
                        # 远距离对准引导（使用更宽松的阈值）
                        angle, offset = self._compute_远_distance_alignment(
                            crosswalk_mask, h, w
                        )
                    ...
                    try:
                        ...
                        if result and "stable_light" in result:
                            ...
                            if stable_light == "go":
                                ...
                                if (
                                    self.green_light_counter
                                    >= GREEN_LIGHT_STABLE_FRAMES
                                ):
                                    self.state = STATE_CROSSING
                                    guidance_text = "绿灯稳定，开始通行。"
                                    self.green_light_counter = 0
                                    self.crossing_end_announced = (
                                        False  # 重置过马路结束标志
                                    )
                                    self.last_crosswalk_seen_time = (
                                        current_time  # 初始化斑马线检测时间
                                    )
                                    self.last_blindpath_announce_time = (
                                        0  # 重置盲道播报时间
                                    )
                                    ...
                                    # 检测到绿灯但还不稳定，节流播报
                                    if (
                                        current_time - self.last_waiting_light_time
                                        > 3.0
                                    ):
                ...
                if TRAFFIC_LIGHT_AVAILABLE and trafficlight_detection:
                    try:
                        ...
                        logger.warning(
                            f"[CROSS_STREET] CROSSING状态红绿灯检测失败: {e}"
                        )
                ...
                if crosswalk_mask is not None:
                    ...
                    if area_ratio > 0.1 and self.crossing_end_announced:
                        ...
                        logger.info(
                            "[CROSS_STREET] 检测到斑马线，重置结束标志，回到正常过马路流程"
                        )

                    # 【移除左上角文字，改为右上角数据面板】
                    panel_data = {"状态": "正在过马路", "面积": f"{area_ratio:.2f}"}
                    ...
                    if stripes and ("angle_deg" in stripes):
                        ...
                        for x1, y1, x2, y2 in stripes.get("lines", []):
                            cv2.line(
                                annotated, (x1, y1), (x2, y2), VIS_COLORS["stripes"], 2
                            )
                    ...
                    if center_pt is not None and stripes and ("angle_deg" in stripes):
                        ...
                        cv2.putText(
                            ...
                            f"{abs(diff):.1f}°",
                            (
                                min(center_pt[0] + 12, w - 110),
                                max(center_pt[1] - 12, 30),
                            ),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.7,
                            (255, 255, 255),
                    ...
                    if current_time - self.last_guide_time > self.guide_interval:
                        # 检查是否快走完斑马线
                        is_almost_done = self._is_crosswalk_almost_done(
                            crosswalk_mask, h, w
                        )
